...
ScTypeAnnotation <- function(obj, assay, f_marker_db, tissue, out_path)
{
    ...
    for(j in unique(sctype_scores$cluster)){
        ...
        obj@meta.data$cell_type2[obj@meta.data$cell_type == celltype] = shortname

        # add cell type 'shortName2' -- 'cell_type3' (for v3)
        #shortname2 = as.character(marker_db$shortName2[marker_db$cellName==celltype][1])
        #obj@meta.data$cell_type3[obj@meta.data$cell_type == celltype] = shortname2
    }
...
## AutoDetect_Tissue
AutoDetect_Tissue <- function(obj, out_path)
{
